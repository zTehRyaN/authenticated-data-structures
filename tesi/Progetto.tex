...
%	Il focus è sulle scelte progettuali. Evidenziare dunque i punti di forza e le scelte a livello di pattern.
	...
	%	Sezione in cui si motiva pattern per interfaccia e si evidenzia collegamento funzionale con casi d'uso e requisiti, in termini solo di funzionalità offerte
		...
		Questa scelta rende l'interfaccia applicabile indipendentemente dalla scelte dei suddetti tipi, caratteristica utile per garantire flessibilità. 
	...
	%	Sezione in cui si motiva pattern per interfaccia e si evidenzia collegamento funzionale con casi d'uso e requisiti, in termini solo di funzionalità offerte
		...
		\begin{itemize}
			\item \textit{rootHash()}: restituisce il root hash
			\item \textit{rootHash(list)}: restituisce il root hash relativo alle coppie chiave-valore presenti in $list$; sovrascrive la proof corrente
		\end{itemize}
