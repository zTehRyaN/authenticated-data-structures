...
%					[Appunti di ragionamento su cloning ADS]
	...
	%	Fatte tutte le opportune premesse descritte e motivate (APS style), si passa ora alla descrizione della struttura skip list proof
		...
		[Discorso per giungere alla somiglianza con skip list]
		
		[Discorso su scelta tra opzione HEAD e opzione KEYS]
		...
		\begin{itemize}
			...
			\item \verb|rootHash(list: List<Pair<K, V?>>): Pair<Proof<H, K, V>, H>|. 
			\item \verb|findNode(key: K, start: AuthSkipListNode<K, H>? = null): AuthSkipListNode<K, H>|. 
%			\item \verb|clone(): Proof<ByteArray, String, Any>|. 
